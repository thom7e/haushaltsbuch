...
def normalize_recurring_rule(raw: dict, account_id: Optional[str] = None) -> dict:
    ...
    if isinstance(raw_months, list):
        ...
        r["months"] = []
    r["manual_label"] = str(r["manual_label"]).strip() if r.get("manual_label") else None
    try:
...
def run_recurring_catchup(user_id: str, data: dict) -> bool:
    ...
    for idx, raw_rule in enumerate(data["recurring_rules"]):
        ...
        is_income = rule["rule_type"] != "Lastschrift"
        ...
        while (y, m) <= (today.year, today.month):
            ...
            if in_range and (y, m) not in existing_months and _should_trigger(rule, y, m) and ym_str not in skipped:
                ...
                if linked:
                    ...
                    raw_amount = abs(float(rule["manual_amount"]))
...
def update_recurring(rule_id: str, payload: dict = Body(...), user=Depends(get_current_user)):
    ...
    if "manual_amount" in payload:
        try:
            cur["manual_amount"] = float(payload["manual_amount"]) if payload.get("manual_amount") is not None else None
        except (TypeError, ValueError):
            pass
